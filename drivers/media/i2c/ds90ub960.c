...
enum ub960_rxport_mode {
	...
	RXPORT_MODE_CSI2 = 3,
	RXPORT_MODE_LAST = RXPORT_MODE_CSI2,
};
...
static void ub960_soft_reset(struct ub960_data *priv);
...
static int ub960_update_bits(const struct ub960_data *priv, u8 reg, u8 mask,
			     u8 val)
{
	struct device *dev = &priv->client->dev;
	int ret;
...
static int ub960_select_ind_reg_block(const struct ub960_data *priv, u8 block)
{
	return ub960_write(priv, UB960_SR_IND_ACC_CTL, block << 2);
}

__maybe_unused
static int ub960_read_ind(const struct ub960_data *priv, u8 reg, u8 *val)
{
	int ret;
	...
	return ub960_read(priv, UB960_SR_IND_ACC_DATA, val);
}
...
static int ub960_write_ind8(const struct ub960_data *priv, u8 reg, u8 val)
{
	...
	return ub960_write(priv, UB960_SR_IND_ACC_DATA, val);
}
...
static int ub960_write_ind16(const struct ub960_data *priv, u8 reg, u16 val)
{
	...
	ret = ub960_write_ind8(priv, reg, val >> 8);
	if (ret)
		return ret;

	return ub960_write_ind8(priv, reg + 1, val & 0xff);
}
...
static int ub960_enable_rx_port(struct ub960_data *priv, unsigned int nport)
{
	...
	if (!rxport->locked) {
		dev_err(dev, "RX port %u not locked\n", nport);
		return -EINVAL;
	}
...
static int ub960_configure_ports(struct ub960_data *priv,
				 struct v4l2_subdev_state *state)
{
	...
	dev_dbg(dev, "ub960_configure_ports\n");
	...
	for (nport = 0; nport < priv->hw_data->num_txports; ++nport) {
		...
		if (!(active_tx_mask & BIT(nport))) {
			ub960_csiport_write(priv, nport, UB960_TR_CSI_CTL, 0);
			continue;
		}
	...
	ub960_soft_reset(priv);

	return 0;
}

static int ub960_enable_streams(struct v4l2_subdev *sd, u32 pad,
				u64 streams_mask)
{
	...
	if (!priv->streaming) {
		ret = ub960_configure_ports(priv, state);
		if (ret) {
...
static int ub960_disable_streams(struct v4l2_subdev *sd, u32 pad,
				 u64 streams_mask)
{
	struct ub960_data *priv = sd_to_ub960(sd);
	struct device *dev = &priv->client->dev;
				...
				struct v4l2_mbus_frame_desc *fd)
{
	...
	for_each_active_route(routing, route) {
		...
		/* Use the RX channel number as VC. See ub960_enable_rx_port() */
...
static void ub960_enable_tpg(struct ub960_data *priv, int tpg_num)
{
	...
	ub960_select_ind_reg_block(priv, UB960_IND_TARGET_PAT_GEN);
...
static void ub960_soft_reset(struct ub960_data *priv)
{
	...
	ub960_write(priv, UB960_SR_RESET, BIT(0));
...
static int ub960_probe(struct i2c_client *client)
{
	...
	dev_info(dev, "Successfully probed %s (rev/mask %#04x)\n",
